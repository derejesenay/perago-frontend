...
const TreeNode: React.FC<TreeNodeProps> = ({ node }) => {
  ...
  return (
    <div className="pl-2 relative bg-cyan-400 pt-2">
      <div className="flex items-center gap-x-1">
        ...
        <span
          ...
          className="cursor-pointer bg-white mb-2 py-1 px-3 w-[400px] border shadow text-sm font-semibold flex items-center gap-x-8"
        >
          {node.name}
          {node.children.length > 0 && (
            <span className="font-normal bg-orange-700 text-white px-4 py-0.5 rounded-md">
      ...
      {!collapsed && (
        <div className="pl-8">
          {node.children.map((childNode, index) => (
            <div key={index}>
              <div className="absolute left-3.5 top-7 bottom-0 w-0.5 bg-red-400"></div>
